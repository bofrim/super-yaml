...
"#;
    let out = proto(doc);
    assert!(out.contains("message Point {"), "missing message block:\n{out}");
    assert!(out.contains("double x = 1;"), "missing x field:\n{out}");
    assert!(out.contains("double y = 2;"), "missing y field:\n{out}");
    assert!(out.starts_with("syntax = \"proto3\";"), "missing proto3 syntax:\n{out}");
}
...
"#;
    ...
    assert!(out.contains("COLOR_UNSPECIFIED = 0;"), "missing unspecified zero:\n{out}");
...
"#;
    let out = proto(doc);
    assert!(out.contains("repeated string tags = 1;"), "missing repeated field:\n{out}");
}
...
"#;
    let out = proto(doc);
    assert!(out.contains("reserved 8;"), "missing numeric reservation:\n{out}");
    assert!(out.contains("reserved \"old_field\";"), "missing string reservation:\n{out}");
    // The removed field itself should NOT be emitted as a normal field
    assert!(!out.contains("old_field = 8"), "removed field should not be a regular field:\n{out}");
}
...
"#;
    let out = proto(doc);
    assert!(out.contains("optional string description = 2;"), "missing optional qualifier:\n{out}");
...
fn generate_proto_types_from_versioned_fields_example() {
    ...
    assert!(out.contains("message Address {"), "missing Address message:\n{out}");
    assert!(out.contains("message UserProfile {"), "missing UserProfile message:\n{out}");
    assert!(out.contains("message BlogPost {"), "missing BlogPost message:\n{out}");

    // old_id was removed in 2.0.0 — should be reserved
    assert!(out.contains("reserved 8;"), "old_id should be reserved at field 8:\n{out}");
    assert!(out.contains("reserved \"old_id\";"), "old_id name should be reserved:\n{out}");
